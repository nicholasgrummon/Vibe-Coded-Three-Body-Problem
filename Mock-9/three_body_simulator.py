...
while not config_ok and tries < MAX_CONFIG_TRIES:
    # 1. Randomize temporary positions within the limit
    temp_pos1 = np.random.uniform(-limit, limit, 2)
    temp_pos2 = np.random.uniform(-limit, limit, 2)
    temp_pos3 = np.random.uniform(-limit, limit, 2)
    
    # 2. Check the "interesting" configuration criteria (separation and non-collinearity)
    # This check is done on the temporary positions as shifting doesn't change distances or area.
    if is_interesting_config(temp_pos1, temp_pos2, temp_pos3):
        
        # 3. Calculate Center of Mass (COM)
        M_total = M1 + M2 + M3
        com_x = (M1 * temp_pos1[0] + M2 * temp_pos2[0] + M3 * temp_pos3[0]) / M_total
        com_y = (M1 * temp_pos1[1] + M2 * temp_pos2[1] + M3 * temp_pos3[1]) / M_total
        com_shift = np.array([com_x, com_y])
        
        # 4. Center the positions by subtracting the COM
        pos1 = temp_pos1 - com_shift
        pos2 = temp_pos2 - com_shift
        pos3 = temp_pos3 - com_shift
        
        # 5. Check if centered positions are still within AXIS_LIMIT
        # We need this check because the shift might push a point originally near the boundary outside.
        if (np.abs(pos1).max() <= limit and 
            np.abs(pos2).max() <= limit and 
            np.abs(pos3).max() <= limit):
...
if tries == MAX_CONFIG_TRIES:
    print(f"Warning: Could not find a COM-centered 'interesting' starting configuration within {MAX_CONFIG_TRIES} tries. Using last attempt.")
else:
    print(f"COM-centered initial configuration found after {tries} attempt(s).")
